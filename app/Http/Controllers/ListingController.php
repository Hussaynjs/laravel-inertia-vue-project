...
class ListingController extends Controller implements HasMiddleware
{
    ...
    public function store(Request $request)
    {
       ...
       $validatedFields = $request->validate([
        ...
        'image' => ['nullable', 'file', 'max:3075', 'mimes:jpg,jpeg']
       ]);

       if($request->hasFile('image')){
       $validatedFields['image'] = Storage::disk('public')->put('image/listing', $request->image);

       }
       $validatedFields['tags'] = implode(array_unique(array_filter(array_map('trim', explode(',' ,  $request->tags)))));
       $validatedFields['approved'] = false;
       $request->user()->listings()->create($validatedFields);
